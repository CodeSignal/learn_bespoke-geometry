...
import { Point, Line, Ray, Segment, Angle, Circle, fromObject, toObject, type GeometryInstance } from './geometry-objects.js';
...
type ToolType = 'point' | 'line' | 'ray' | 'segment' | 'angle' | 'circle' | 'intersection' | 'remove';

export interface ObjectEntry {
  obj: GeometryInstance;
  name?: string;
  /** Label placement angle in radians (0 = right); used for point labels to avoid overlap. */
  labelAngle?: number;
}

/** Serializable operation log entry for validation. One JSON object per server log line. */
...
function pushLog(entry: OperationLogEntry): void {
...
function findExistingPointNear(x: number, y: number): Point | null {
...
function findObjectAt(x: number, y: number): HitResult | null {
  ...
  if (best.obj.type === 'line' || best.obj.type === 'ray' || best.obj.type === 'segment') {
    ...
    for (const [ex, ey] of endpts) {
      ...
      const pointEntry = state.objectEntries.find(
        (e) => e.obj.type === 'point' && Math.abs((e.obj as Point).x - ex) < 1e-9 && Math.abs((e.obj as Point).y - ey) < 1e-9
      );
      if (pointEntry) {
...
function sameSelectedObject(a: GeometryInstance, b: GeometryInstance): boolean {
  const ao = toObject(a), bo = toObject(b);
  if (ao.type !== bo.type) return false;
  if (ao.type === 'point' && bo.type === 'point') return Math.hypot(ao.x - bo.x, ao.y - bo.y) < 1e-9;
  if (ao.type === 'circle' && bo.type === 'circle') return Math.abs(ao.cx - bo.cx) < 1e-9 && Math.abs(ao.cy - bo.cy) < 1e-9 && Math.abs(ao.r - bo.r) < 1e-9;
...
let panStart: { vb: ViewBox; clientX: number; clientY: number; rect: DOMRect } | null = null;
...
function render(): void {
  ...
  if (tool === 'point' && (state.hoverPoint || hoverRaw)) {
    ...
    if (hoverRaw && onLineLike) {
      ...
      if (o.type === 'line') h = geom.closestPointOnLine(hoverRaw.x, hoverRaw.y, o.x1, o.y1, o.x2, o.y2);
      else if (o.type === 'ray') h = geom.closestPointOnRay(hoverRaw.x, hoverRaw.y, o.x1, o.y1, o.x2, o.y2);
      else if (o.type === 'segment') h = geom.closestPointOnSegment(hoverRaw.x, hoverRaw.y, o.x1, o.y1, o.x2, o.y2);
      else if (o.type === 'circle' && o.r > 1e-9) h = geom.closestPointOnCircle(hoverRaw.x, hoverRaw.y, o.cx, o.cy, o.r);
...
function labelOverlapCount(lx: number, ly: number): number {
  let count = 0;
  for (const { obj } of state.objectEntries) {
    if (obj.type === 'line') {
      if (geom.distToLine(lx, ly, obj.x1, obj.y1, obj.x2, obj.y2) < LABEL_OVERLAP_THRESHOLD) count++;
    } else if (obj.type === 'ray') {
      if (geom.distToRay(lx, ly, obj.x1, obj.y1, obj.x2, obj.y2) < LABEL_OVERLAP_THRESHOLD) count++;
    } else if (obj.type === 'segment') {
      if (geom.distToSegment(lx, ly, obj.x1, obj.y1, obj.x2, obj.y2) < LABEL_OVERLAP_THRESHOLD) count++;
    } else if (obj.type === 'angle') {
...
function recomputeLabelAngles(): void {
  for (const entry of state.objectEntries) {
    ...
    if (labelOverlapCount(lx, ly) === 0) continue;
    let bestAngle = angle;
...
function setLabelContent(textEl: SVGTextElement, name: string): void {
...
function getPointLabelPosition(entry: ObjectEntry): { x: number; y: number; anchor: string } {
...
function performClickAt(pt: { x: number; y: number }): void {
  ...
  if (tool === 'remove') {
    if (hit) {
      ...
      else if (state.selectedObjectIndex !== null && state.selectedObjectIndex > hit.index) state.selectedObjectIndex--;
  ...
  if (tool === 'point') {
    ...
    if (hit && hit.obj.type !== 'point') {
      const o = hit.obj;
      if (o.type === 'line') position = geom.closestPointOnLine(pt.x, pt.y, o.x1, o.y1, o.x2, o.y2);
      else if (o.type === 'ray') position = geom.closestPointOnRay(pt.x, pt.y, o.x1, o.y1, o.x2, o.y2);
      else if (o.type === 'segment') position = geom.closestPointOnSegment(pt.x, pt.y, o.x1, o.y1, o.x2, o.y2);
      else if (o.type === 'circle' && o.r > 1e-9) position = geom.closestPointOnCircle(pt.x, pt.y, o.cx, o.cy, o.r);
    }
...
function setTool(tool: string): void {
  state.currentTool = tool as ToolType;
  state.pendingPoints = [];
  state.pendingIntersectionObjects = [];
...
/** Snapshot of the plane for validation: same shape as saved state. */
...
function loadState(): void {
  try {
    ...
    for (const item of parsed) {
      const geom = item && (item.geom != null ? item.geom : item) as GeomObject;
