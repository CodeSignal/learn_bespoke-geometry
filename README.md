...
You can run validation (e.g. `npm run validate`); set `LOG_FILE` for a different log path. Snapshot or stdin: `node run-validate.mjs snapshot.json` or pipe JSON. Exit code 0 = valid, 1 = invalid. Details and payload formats: [AGENTS.md](./AGENTS.md).
