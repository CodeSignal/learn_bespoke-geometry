...
exports.DEFAULT_TASK_ID = exports.TASK_REGISTRY = void 0;
exports.validateLineWithPointsTask = validateLineWithPointsTask;
exports.validateEquilateralTriangleTask = validateEquilateralTriangleTask;
exports.validateParallelLinesTask = validateParallelLinesTask;
exports.getTaskIds = getTaskIds;
...
function validateLineWithPointsTask(snapshot) {
    ...
    for (const entry of points) {
        ...
        const d = (0, geometry_js_1.distToLine)(x, y, x1, y1, x2, y2);
        if (d <= POINT_ON_LINE_TOLERANCE)
            onLine++;
        else
            offLine++;
    }
...
function validateEquilateralTriangleTask(snapshot) {
    if (!Array.isArray(snapshot)) {
        return { valid: false, message: 'Invalid snapshot: expected an array.' };
    }
    ...
    const lengths = segments.map((e) => (0, geometry_js_1.segmentLength)(e.geom));
    const [a, b, c] = lengths;
    if (Math.abs(a - b) > LENGTH_TOLERANCE ||
    ...
    const endpoints = segments.map((e) => (0, geometry_js_1.getEndpoints)(e.geom)).filter(Boolean);
    if (endpoints.length !== 3)
        return { valid: false, message: 'Invalid segment geometry.' };
...
function validateParallelLinesTask(snapshot) {
    if (!Array.isArray(snapshot)) {
        return { valid: false, message: 'Invalid snapshot: expected an array.' };
    }
    const lineLikes = snapshot.filter((e) => e?.geom && (0, types_js_1.isLineLike)(e.geom));
    ...
    if (!(0, geometry_js_1.areParallel)(first, second)) {
        return {
            valid: false,
...
exports.DEFAULT_TASK_ID = 'line-and-points';
function getTaskIds() {
    return Object.keys(exports.TASK_REGISTRY);
}
/** Run the validator for the given task id; falls back to default if unknown. */
function runTask(taskId, snapshot) {
    const fn = exports.TASK_REGISTRY[taskId] ?? exports.TASK_REGISTRY[exports.DEFAULT_TASK_ID];
    return fn(snapshot);
}
function distinctPoints(points, tolerance) {
    const result = [];
    for (const p of points) {
        if (!result.some((q) => (0, geometry_js_1.distanceBetweenPoints)(p.x, p.y, q.x, q.y) <= tolerance)) {
            result.push(p);
        }
    }
    return result;
